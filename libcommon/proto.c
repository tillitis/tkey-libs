...
// read_frame reads an entire frame according to the "framing protocol".
...
int read_frame(uint8_t *buf, size_t bufsize, struct frame_header *hdr)
{
	...
	if (*ver >= TKEY_VERSION_CASTOR) {
		if (read_serial(&in, 1, 1) < 0) {
			return -1;
		}
	} else {
		if (read(IO_UART, &in, 1, 1) < 0) {
			return -1;
		}
	}
	...
	if (bufsize < hdr->len) {
		...
		if (*ver >= TKEY_VERSION_CASTOR) {
			discard_serial(hdr->len);
		} else {
			discard(IO_UART, hdr->len);
		}

		return -2; // invalid frame dropped
	}

	if (*ver >= TKEY_VERSION_CASTOR) {
		return read_serial(buf, bufsize, hdr->len);

	} else {
		int ret = read(IO_UART, buf, bufsize, hdr->len);
		return ret;
	}
